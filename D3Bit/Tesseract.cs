...
//      private static Hunspell hunspell = new Hunspell("en_US.aff", "en_US.dic");
        ...
        public static string GetTextFromBitmap(Bitmap bitmap, string extraParams)
        {
            ...
            bitmap.Save(Path.Combine(currentDir, "tesseract", "x.png"), ImageFormat.Png);
            //sw.Lap("File Save");
...
//          ProcessStartInfo info = new ProcessStartInfo(Path.Combine("tesseract", "tesseract.exe"), Path.Combine("tesseract", "x.png") + " " + Path.Combine("tesseract", "x") + extraParams);
            ProcessStartInfo info = new ProcessStartInfo(Path.Combine(currentDir, "tesseract", "tesseract_mac"), Path.Combine(currentDir, "tesseract", "x.png") + " " + Path.Combine(currentDir, "tesseract", "x") + " " + extraParams);
            info.WindowStyle = ProcessWindowStyle.Hidden;
            Process p = Process.Start(info);
            p.WaitForExit();
//          //sw.Lap("Tesseract");

            TextReader tr = new StreamReader(Path.Combine(currentDir, "tesseract", "x.txt"));
